...
Rails Network Swap rewards users to contribute liquidity to its trading pools by offering them a share of the transaction fees generated from trades executed within those pools. Generally, providing liquidity and market making can be intricate, carrying the risk of financial loss, especially during significant and prolonged price fluctuations of the underlying assets, as opposed to merely holding the assets.

# Risks

As with any trading protocol, providing liquidity comes with its own risks. Below is an example of risks involved with participating in Liquidity Pools.
...
**Now let’s assume the price trades on Coinbase from $100 to $150. The Rails Network Swap contract should reflect this change as well after some arbitrage. Traders will add DAI and remove ETH until the new ratio is now 150:1.**
...
To understand why the value of a liquidity provider’s stake can go down despite income from fees, we need to look a bit more closely at the formula used by Rails Network Swap to govern trading. The formula really is very simple. If we neglect trading fees, we have the following:
